...
echo "blastx -db ${BlastDB} -query fasta/${name}.fa -out blastout/${name}.blastout -num_threads ${Threads} -max_target_seqs 1 -evalue 0.00001 -outfmt \"6 qseqid sseqid pident qlen length mismatch gapopen qstart qend sstart send slen nident evalue bitscore qcovs\"" >> shells/${name}.sh
